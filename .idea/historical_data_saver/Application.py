...
logging.basicConfig(filename="Application_logs/App_Main_logs.log",
                    ...
                    # Set the date format for timestamps  # Set the filename for the log file
                    )
...
fileHandler = TimedRotatingFileHandler(filename="Application_logs/App_Main_logs.log", when="midnight", interval=1,
                                       backupCount=30, )
...
def get_last_price():
    ...
    if 'p' in trade_data:
        ...
        last_price = float(
            trade_data['p'])  # assigns the value of the p key in the trade_data dictionary to the last_price variable.
...
def startWebSocket(currency_pair):
    ...
    while True:
        ...
        try:
            url = f"wss://stream.binance.com/ws/{currency_pair}@aggTrade"
            ws = websocket.WebSocketApp(url,  # The URL of the Binance websocket server.
                                        on_open=on_open,
                                        # A callback function that is called when the websocket is opened.
                                        on_message=on_message,
                                        # A callback function that is called when the websocket receives a message.
                                        on_error=on_error,
                                        # A callback function that is called when the websocket encounters an error.
