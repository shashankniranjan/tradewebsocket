...
logging.basicConfig(filename="crypto_data_logger/App_starter_logs/App_logs.log", level=logging.INFO, format=f"%(levelname)s - %(msecs)d - %(message)s")
# Create a TimedRotatingFileHandler object
file_handler = TimedRotatingFileHandler("crypto_data_logger/App_starter_logs/App_logs.log", when="midnight", interval=1, backupCount=1000000000)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.error(f"Error reading configuration files:{currencies} {e}")
